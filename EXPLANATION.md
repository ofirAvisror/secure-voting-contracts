...
## 6. The tests (`test/election.test.js`)

15 tests (Hardhat + Chai) covering:
- The token: correct name/symbol, and only the minter can mint.
- Admin: only the owner can add candidates, rejecting out-of-range positions, storing the voter book, rejecting invalid time windows.
- Direct voting: counting a valid vote + minting reward and receipt, rejecting an ineligible voter, preventing double voting, and enforcing the time window (before/after).
- Anonymous voting: picking the nearest candidate, and rejecting out-of-range answers.
- Soulbound receipt: cannot be transferred.
- Results: correct sorted ranking and winner.

Run with: `npm test` (all 15 pass).

---

## 7. End-to-end workflow (running locally)
...
## 8. Deploying to Sepolia (a real testnet)
